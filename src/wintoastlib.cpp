...
namespace DllImporter
{
    ...
    template<typename T>
    inline HRESULT
    Wrap_GetActivationFactory(_In_ HSTRING activatableClassId, _Inout_ Details::ComPtrRef<T> factory) throw()
    {
...
namespace Util
{
    inline HRESULT defaultExecutablePath(_In_ LPSTR path, _In_ DWORD nSize = MAX_PATH)
    {
    ...
    {
        DWORD written = GetEnvironmentVariable("APPDATA",
                                               (LPSTR) aperstring::convertWStringToString(std::wstring(path)).c_str(), nSize);
    ...
    inline HRESULT
    setEventHandlers(_In_ IToastNotification *notification, _In_ std::shared_ptr<IWinToastHandler> eventHandler)
    {
...
HRESULT WinToast::validateShellLinkHelper()
{
    ...
    DWORD attr = GetFileAttributes((LPSTR) aperstring::convertWStringToString(std::wstring(path)).c_str());
    if (attr >= 0xFFFFFFF)
    {
...
HRESULT WinToast::setImageFieldHelper(_In_ const std::wstring &path)
{
    ...
    HRESULT hr = StringCchCat((LPSTR) aperstring::convertWStringToString(std::wstring(imagePath)).c_str(), MAX_PATH, (LPSTR) aperstring::convertWStringToString(std::wstring(imagePath)).c_str());
    if (SUCCEEDED(hr))
    {
        ComPtr <IXmlNodeList> nodeList;
